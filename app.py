...
from src.sapi import fetch_jobs_from_serpapi
...
if uploaded_file:
    ...
    """
        ...
        if jobs:
                    ...
                    for job in jobs:
                        st.write(f"**Keyword:** {job['keyword']}")
                        st.write(f"**Location:** {job['location_used']}")
                        st.markdown(f"[Google Jobs Link]({job['google_jobs_url']})")
        else:
